...
const ROWS : int = 14
const COLS : int = 15
const CELL_SEZE : int = 50

# 타일맵 변수
var tile_id : int = 0	# 타일셋에 있는 아틀라스 ID

# 레이어, 인스펙터에 있는 타일멥 레이어와 같은 순서
...
func _input(event):
	if event is InputEventMouseButton:
		if event.position.y < ROWS * CELL_SEZE:
...
func process_left_click(pos):
	...
	while not cells_to_reveal.is_empty():
		...
		revealed_cell.append(cells_to_reveal[0])
		...
		if not is_number(cells_to_reveal[0]):
			cells_to_reveal = reveal_surrounding_cells(cells_to_reveal, revealed_cell)
